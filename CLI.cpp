...
void CLI::listAccessPoints() {
    ...
    for(const std::pair<std::string, std::set<address_type>>& pair: aps) {
        ...
        for(std::set<address_type>::iterator it = pair.second.begin(); it != pair.second.end(); it++) {
...
void CLI::chooseAccessPoint(int no) {
    int i = 1;
    for(std::map<std::string, std::set<address_type>>::iterator iterator = aps.begin(); iterator != aps.end(); iterator++, i++) {
        if(i == no) {
